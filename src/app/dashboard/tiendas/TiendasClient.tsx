      ...
      {stores.length === 0 ? (
        <div className="text-center py-12">
          <div className="bg-gray-50 rounded-lg p-8">
            <p className="text-gray-600 text-lg mb-4">No hay tiendas registradas aún</p>
            <Button
              onClick={() => router.push("/dashboard/tiendas/add")}
              className="bg-blue-600 hover:bg-blue-700 text-white"
            >
            ...
            <Card key={store.id} className="p-6 hover:shadow-lg transition-shadow">
              ...
              {store.inventoryManager && (
                <div className="mt-4 pt-4 border-t border-gray-200">
                  <p className="text-xs text-gray-500 mb-1">Administrador de Inventario</p>
                  <p className="text-sm font-medium text-gray-700">{store.inventoryManager.user.name}</p>
                </div>
              )}
