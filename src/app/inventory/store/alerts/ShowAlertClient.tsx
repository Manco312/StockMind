...
export default function ShowAlertClient({ userType, userName, storeId }: ShowAlertClientProps) {
  ...
  const expiredBatchAlerts = alerts.filter((a) => a.type === "EXPIRED_BATCH")
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            {lowStockAlerts.length > 0 && (
              <div>
                <h2 className="text-xl font-semibold text-gray-800 mb-4">Alertas de Stock Bajo</h2>
                  ...
                  {lowStockAlerts.map((alert) => (
                    ...
                    >
                      <div>
                        ...
                        {alert.resolved && (
                          <p className="text-sm text-green-700 mt-2 font-medium">✅ Alerta resuelta</p>
                        )}
                      </div>
            ...
            {expiredBatchAlerts.length > 0 && (
              <div>
                <h2 className="text-xl font-semibold text-gray-800 mb-4">Alertas de Lotes Vencidos</h2>
                <div className="grid gap-4">
                  {expiredBatchAlerts.map((alert) => (
                    <div
                      key={alert.id}
                      className={`border rounded-lg p-4 shadow-sm flex justify-between items-center ${
                        alert.resolved ? "bg-green-50 border-green-200" : "bg-orange-50 border-orange-300"
                      }`}
                    >
                      <div>
                        <p className="font-semibold text-gray-700">{alert.message}</p>
                        <p className="text-sm text-orange-700 mt-1">
                          ⚠️ Este lote ha expirado y debe ser retirado del inventario
                        </p>
